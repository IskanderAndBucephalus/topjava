...
public class MealsUtil {
    public static final Integer MAX_CALORIES = 2000;

    public static List<MealWithExceed> getFilteredWithExceededCustom(Collection<Meal> meals, int caloriesPerDay) {
        {
            Map<LocalDate, Integer> groupedDays = meals.stream().collect(groupingBy(userMeal -> userMeal.getDateTime().toLocalDate(),
                    summingInt(Meal::getCalories)));

            return meals.stream()
                    //.filter(um -> TimeUtil.isBetween(um.getDateTime().toLocalTime(), startTime, endTime))
                    .map(um -> createWithExceed(um, groupedDays.get(um.getDateTime().toLocalDate()) > caloriesPerDay))
                    .collect(toList());
        }
    }
...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static MealWithExceed createWithExceed(Meal meal, boolean exceeded) {
        MealWithExceed object = new MealWithExceed(meal.getDateTime(), meal.getDescription(), meal.getCalories(), exceeded);
        object.setId(meal.getId());
        return object;
    }
}